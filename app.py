...
SUPPORTED_TEXT_MODELS = {
    ...
    "mistral": {"repo_id": "TheBloke/Mistral-7B-Instruct-v0.2-GGUF",
...
class Api:
    ...
    def _download_worker(self):
        ...
        api = HfApi()
        all_repos = set(config['repo_id'] for config in {**SUPPORTED_TEXT_MODELS, **SUPPORTED_IMAGE_MODELS}.values())
            ...
            try:
                ...
                for filename in repo_files:
                    if not filename.startswith('.') and not filename.endswith('/'):
                        files_to_download.append((repo_id, filename))
        ...
        for repo_id, filename in files_to_download:
            ...
            try:
                if window:
                    window.evaluate_js(f'updateDownloadProgress("{message}", {progress + 1}, {total_files})')
    ...
    def initialize_models(self, text_model_id, image_model_id):
        ...
        try:
            ...
            error_str = f"Failed to load models. Ensure they were downloaded. Error: {e}"
